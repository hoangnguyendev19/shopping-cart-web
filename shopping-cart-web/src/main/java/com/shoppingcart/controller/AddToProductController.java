...
@WebServlet("/views/add-to-product")
public class AddToProductController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			boolean result = pd.addToProduct(pm);
			
			if(result) {
				response.sendRedirect("./product-admin.jsp");
			} else {
				response.getWriter().print("Thêm sản phẩm thất bại! Bạn hãy thực hiện lại!");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
